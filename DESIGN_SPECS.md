...
>
> ⚠️ **复核说明（2026-08-01 再次逐条对照当前代码）：** 下表以**当前代码状态**为准。标注「**已解决**（原 Px）」的行，其修复已在最新提交中落地（保留用于追溯与回归验证）；其余为仍待决项。原审计中若干 P1 已关闭，剩余风险以 P2 为主，决策顺序见 §8.3。本轮快速修复批次（UX-02/03/05/07/08、SEC-08/09）已随审计修复计划落地并于同日复核为已解决。
...
| SEC-08 | **已解决**（原 P2） | 复核确认：`music_cache` 已有 `expires_at` 列 + 索引，读写均按 TTL 判过期；`cleanupMusicCache()` 已实现——启动时（`server.ts`）与每 10 分钟定时删除过期行，并按 `MUSIC_CACHE_MAX_ROWS=5_000` 保留最新行（SEC-08 修复已落地）。残余：`value_json` 仍会持久化解析出的音频 URL（含短期签名 URL）与歌词，"只缓存最小必要元数据"建议未执行；备份策略仍建议加密或排除该缓存。 | `backend/src/services/musicCache.ts` 的 `cleanupMusicCache()`/`MUSIC_CACHE_MAX_ROWS`；`backend/src/server.ts` 的启动调用与 `setInterval`；`backend/src/db/schema.sql` 的 `music_cache`。 | 已完成：定时删除过期行 + 5k 行上限。残余：只缓存最小必要元数据（避免落盘临时签名 URL/敏感 token）；备份策略加密或排除该缓存。 |
| SEC-09 | **已解决**（原 P2） | 复核确认：`.gitignore` 已移除 `!data/twice.db` 反向规则，`data/twice.db` 空占位文件不再被 Git 跟踪（`git ls-files` 无任何 `.db`）；`backend/data/` 忽略规则保留，真实运行时库与 `.bak` 不进入仓库。残余：备份/部署打包仍需确认排除 `backend/data/` 与 `.bak`；pre-commit/CI secret 与二进制 DB 检测未实施；本地 `.env` 弱开发密码仍建议轮换（见 SEC-01）。 | `.gitignore`（无 `!data/twice.db`）；`git ls-files`（无 `data/twice.db`）；`backend/data/` 忽略规则。 | 已完成：移除反向规则并取消跟踪占位文件。残余：备份/部署包排除检查与 secret/DB 检测；旧部署凭据轮换。 |

### 8.2 样式、可访问性与产品质量

| ID | 严重度 | 已验证问题 | 证据 | 建议处置 |
| --- | --- | --- | --- | --- |
| UX-01 | **已解决**（原 P1） | 复核确认：全前端唯一 `<audio>` 在 `MiniAudioBar.vue`；`MusicStationView` 已不再渲染第二 `<audio>`，播放经 `audioStore.playResolvedMusic()` 载入全局链路（store 内另有 ≤16 个仅 `preload='metadata'` 的瞬时暖机 `Audio` 对象，不构成第二播放链路）；`.music-station-audio`/`.music-station-lyrics-empty` 死样式已删除。残余：补 E2E 验证切换后无第二路播放。 | `frontend/src/views/MusicStationView.vue`（无 `<audio>`，调用 `playResolvedMusic`）；`frontend/src/components/player/MiniAudioBar.vue`（唯一 `<audio>`）；`frontend/src/stores/audio.ts` 的 `warmAudio()`。 | 已完成主体重构与死样式清理；补 E2E。 |
| UX-02 | **已解决**（原 P1） | 复核确认：`admin-login-link`/`admin-account-button`/`admin-auth-slot` 已全部收敛到 `admin.css` 单一所有权，`global.css` 中不再定义（重复选择器扫描为 0 处）；`admin-page`/`admin-panel`/`admin-table` 亦仅存在于 `admin.css`。残余：禁止全局裸 `.n-button`/`.n-tag` 覆盖见 UX-06（仍待决）；浅/暗/窄屏视觉回归未补跑。 | `frontend/src/styles/admin.css`（2738 行起定义 admin-account-button/admin-auth-slot/admin-login-link）；`frontend/src/styles/global.css`（已无 admin-login-link 等规则）。 | 已完成：admin 外壳样式收敛到单一所有权。残余：UX-06 全局覆盖重构与视觉回归。 |
| UX-03 | **已解决**（原 P2） | 复核确认：`global.css` 已新增全局 `prefers-reduced-motion` 规则（第 45 行起），覆盖公开页全部元素；首页轮播 `startCarousel()`、Hero 背景视频 `startHeroVideo()`、MV 预热 `shouldPrewarmMv()` 均显式检查 reduce 后停止自动行为。残余：scoped `LyricsDisplay` 的 `fadeInUp`/过渡无组件内 reduce 分支（由全局 `*` 规则兜底收敛，非无限动画）；`MusicStationView` 结果卡 hover 的 `translateY(-3px)` 位移未在 reduce 下显式禁用（全局规则只收敛了 transition 时长，位移本身仍在 reduce 下瞬间生效，§3.1.1 要求桌面端同样禁用）；建议后续对组件 scoped 动效逐一复核。 | `frontend/src/styles/global.css`（45 行起全局 reduce 规则）；`frontend/src/views/HomeView.vue`（startCarousel/startHeroVideo/shouldPrewarmMv 的 reduce 检查）；`frontend/src/components/player/LyricsDisplay.vue`（fadeInUp）；`frontend/src/views/MusicStationView.vue`（`.music-result-card:hover` 的 translateY）。 | 已完成：全局 reduce 规则 + 轮播/背景视频/预热停止。残余：MusicStationView hover 位移与 scoped 动效逐个复核、无限装饰动画清理。 |
...
| UX-05 | **已解决**（原 P2） | 复核确认：首页精选 MV 轮播已支持键盘前后切换（容器 `tabindex=0` + 左右方向键）、可见暂停/继续按钮（`aria-pressed`）、圆点 `aria-current`、i18n slide 文案（`home.slideAria`），且 `prefers-reduced-motion` 或暂停态下停止自动切换（`startCarousel()` 检查 `carouselPaused` 与 reduce）。残余：hover 离开后 `resumeCarousel()` 会恢复自动播放，未做"焦点/hover 后不重新强制自动播放"策略。 | `frontend/src/views/HomeView.vue`（tabindex、keydown.left/right、pause 按钮、aria-current、slideAria、reduce 检查）。 | 已完成：键盘/暂停/aria-current/i18n/reduce。残余：焦点或 hover 后不应重新强制自动播放。 |
...
| UX-07 | **已解决**（原 P2） | 复核确认：`AppShell` 主题切换已改可访问 SVG（☀/☾ emoji 移除）；首页轮播圆点 aria 已用 i18n `home.slideAria`；`AdminMvsView` BVID/页码/链接 placeholder 与 `MemberDetailView` MBTI 标签已入消息表；新增 i18n 完整性单元测试（五语言无缺 key，见 `messages.test.ts`）。残余：`VarietyView` 大量硬编码内容归属 UX-04（待产品决策）；CI key 完整性/硬编码扫描未加。 | `frontend/src/components/layout/AppShell.vue`（SVG 图标）；`frontend/src/views/HomeView.vue`（`home.slideAria`）；`frontend/src/views/AdminMvsView.vue`（`admin.mvs.bvidPlaceholder`）；`frontend/src/views/MemberDetailView.vue`（`member.mbti`）；`frontend/src/i18n/messages.test.ts`。 | 已完成：SVG/i18n 清理与完整性测试。残余：UX-04 与 CI 硬编码扫描。 |
| UX-08 | **已解决**（原 P3） | 复核确认：`--shadow-soft` 已在 `global.css` 浅色（`:root`）与暗色（`:root[data-theme='dark']`）两侧定义，`MusicStationView` 等引用处不再退化；`--accent-gradient` 已无引用（`--admin-accent-gradient` 仅在 admin.css 定义并使用）。残余：`--member-color` 仍为全局引用、仅成员卡局部赋值（设计内用法）；tokens 文件与 lint 未建立。 | `frontend/src/styles/global.css`（`--shadow-soft` 定义）；`frontend/src/views/MusicStationView.vue`（`var(--shadow-soft)` 使用）。 | 已完成：token 补齐。残余：tokens 文件/lint 与 `--member-color` 收敛。 |
...
1. **已完成（复核确认，无需再批准）：** SEC-01、SEC-02、SEC-04、SEC-05、UX-01 已落地；SEC-03 主体（签名+限速）已落地，仅剩并发/带宽配额残余；UX-02、UX-03、UX-05、UX-07、UX-08、SEC-08、SEC-09 已随审计 quick-wins 落地（2026-08-01 复核确认）。
2. **必须优先批准：** UX-10（MV 首帧性能，§6.4）、SEC-03 残余（并发/带宽配额与全局限速）。
3. **应在任何新增后台/播放能力前批准：** SEC-06、SEC-07、UX-04、UX-06。
4. **可作为设计债务排期：** UX-09。

剩余风险以 P2 为主；公开部署后台/启用 B站流代理/扩展音乐下载缓存能力前，仍应先完成 §5.2 写操作审计与 SEC-07 last-owner 保护。
