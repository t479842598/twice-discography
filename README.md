...
### 2026-08-01（MV 声音与画质修复）

- **修复 MV 无声：** 内置流代理响应此前缺少 `Vary: Range`，浏览器缓存把首个 2MB 分块当作所有 Range 请求的结果返回，导致 DASH 分块拉取错乱、音视频双轨起播失败；现补上 `Vary: Range` 并将缓存时长对齐签名有效期（10 分钟）。
- **修复 MP4 兜底静音：** 兜底路径此前误用 DASH 视频轨（只有画面没有音轨），回退后依然无声；新增 `kind=mp4` 流模式，兜底时返回含音轨的 durl MP4。
- **移除播放通道选择器：** 去掉"播放通道（自动 / 官方播放器）"开关及对应五语言文案；解析失败时仍自动回退官方播放器。
- **画质最低 1080P：** 接口与播放器均把可选画质限制为 1080P 及以上（源视频仅支持更低时退回最高可用）；起播即 1080P，不再 480P 起播后自动升级，并忽略历史保存的低画质偏好。

### 2026-07-31（稳定性加固）

- **修复音乐站搜索异常：** 后端进程曾因 MV 流式转发时的流错误而退出，导致音乐站等所有接口连接失败；已为视频/封面流转发添加流错误处理，客户端断开不再影响进程。
- 音乐搜索接口增加 15 秒兜底超时，上游音源异常时按时返回已有结果而非挂起。
- 后端增加全局 unhandledRejection / uncaughtException 守护，异常只记日志不宕机。


### 2026-07-31（MV 声音修复）

- **修复 MV 无声问题：** DASH 播放内核曾因 mp4box 默认丢弃 mdat 媒体数据导致只有画面没有声音；现显式保留数据，音视频双流正常合并。
- **清晰度切换完善：** 720P/480P/360P 也统一走 DASH 播放（此前低清晰度回退 MP4 因接口参数不兼容而失效），支持在 4K 加载中切换其它画质（最新选择优先）。
- MSE 不可用时仍回退 720P MP4（`format=mp4` 显式请求，不再误用 DASH 接口）。
- 默认播放 1080P，4K 可选；已用真实浏览器验证：1080P / 720P / 4K 切换均有声音、分辨率正确。

### 2026-07-31（MV 高清与清晰度）

- **MV 高清化：** 播放接口改用 B站 DASH 格式，1080P/4K 可用；实测 ABC D MV 可播放 4K（3840×2160）。
- **清晰度选择器：** 播放器内新增画质选择（4K / 1080P60 / 1080P 高码率 / 1080P / 720P / 480P / 360P），切换即时生效；源视频仅支持到 720P 时自动降级。
- **DASH 播放内核：** 前端用 mp4box.js + MediaSource 实时合并视频/音频流（B站官方播放器同款方案）；MSE 不可用或失败时自动回退 720P MP4，不会黑屏。
- 内置流代理签名 token 绑定清晰度与音/视频流类型；`/playback?qn=` 支持指定画质。
- 修复单体部署时前端构建误用线上 API 地址的问题（`frontend/.env.production` 默认改为同源 `/api`）。
...
- 新增后台登录、管理员会话、角色权限、管理员新增/编辑与密码修改能力。
- 新增右上角登录态入口：未登录显示登录按钮，登录后显示当前账号、后台菜单和退出登录。
- 新增 MV / B站管理后台，支持 BVID 配置、B站凭证保存与校验、MV 播放代理配置。
- 新增 Cloudflare Worker MV 代理示例，后端签发短时效代理 URL，避免把 B站 Cookie 下发到浏览器。
- 新增歌词数据库缓存，播放解析拿到 LRC 后写入 SQLite，R2 音频缓存命中时也会从数据库补回歌词。
- 优化后台管理页样式，用户管理改为弹框新增和弹框编辑。
- 修复首页歌曲列表封面尺寸异常，避免封面和文字重叠。
- 更新部署环境变量、Render 配置和后台 / MV 代理相关说明。
